...
void setup()
{
  attachInterrupt(2, Sbros, RISING);
  attachInterrupt(0, Plus, RISING);
  attachInterrupt(0, Pl, FALLING);
  attachInterrupt(1, Minus, RISING);
  attachInterrupt(1, Mn, FALLING);
  //////
  Serial.begin(9600);
  Porog = EEPROM.read(0);
  if(Porog == 255) Porog = 500;
  else Porog*=10;
  delay(1000);
...
/////////////////////////////////////////////////////////////////
volatile unsigned long Pmil = 0;
void Plus()
{
  if(Pmil + 100 <= millis())
  {
   if(Porog < 1020) Porog+=10;
   EEPROM.write(0, byte(Porog/10));
   Pmil = millis();
 Serial.println(Porog);
  /////////////////////////////// 
  }
} 
void Pl(){if(Pmil + 100 <= millis()) Pmil = millis();}

volatile unsigned long Mmil = 0;
void Minus()
{
    if(Mmil + 100 <= millis())
  {
...
void Mn(){if(Mmil + 100 <= millis()) Mmil = millis();}
